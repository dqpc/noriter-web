...
| 계단 오르기 | Shift 방향전환, Space 오르기 / 좌우 버튼 | 에너지 바(초반 30칸은 천천히 줄어듦), 번개 아이템, 10칸마다 보너스 에너지, 대시(방향전환 직후 오르기, 시작 전 팁으로 안내). 이정표 연출(10칸마다 COMBO, 대시 PERFECT!, 번개 GOOD!)은 점수와 무관 |
...
- **카톡으로 공유**: 글딱지 결과 화면과 방 대기실에 카카오톡 공유 버튼(피드 카드, `og.png` 미리보기). `VITE_KAKAO_JS_KEY` 가 없는 빌드에서는 버튼이 숨는다. 계정으로 공유한 글딱지 카드 링크에는 `?by=<내 id>` 가 붙어, 받은 사람이 들어오면 보낸 사람을 친구로 추가하는 배너가 뜬다(게스트에게는 가입 안내).
