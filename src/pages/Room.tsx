...
export function Room() {
  ...
  const send = useCallback(
    (msg: Parameters<RoomSocket["send"]>[0]) => socketRef.current?.send(msg),
    [],
  );
    ...
    <section className="page room">
      <div className="play-bar">
        ...
        <span className="play-best room-me">
          <CharacterAvatar id={me?.character ?? getMyCharacter()} size={28} />{" "}
          {me?.nickname ?? nickname}
        </span>
      </div>
      {error && <p className="room-error">{error}</p>}
      {disconnected && (
        <p className="room-error">연결이 끊어졌습니다. 새로고침해 주세요.</p>
      )}
      ...
      {room.status === "COUNTDOWN" && (
        <div className="room-countdown">
          ...
          <p className="room-countdown-num">
            {Math.max(
              0,
              Math.ceil((new Date(room.startAt ?? 0).getTime() - now) / 1000),
            )}
          </p>
        </div>
      )}
          ...
          <div className="room-play">
            ...
            <game.Component
              ...
              options={{
                ...room.options,
                seed: room.seed,
                character: me?.character ?? getMyCharacter(),
              }}
            />
            <Scoreboard
...
function Lobby({
  ...
  const share = () =>
    navigator
      .share({ title: `${room.game.name} 함께 하기`, url: inviteUrl })
      .catch(() => {});
      ...
      <div className="room-settings">
            ...
            <div className="g2048-targets">
              {choices.map((c) => (
                <button
                  ...
                  type="button"
                  className={
                    String(room.options[key]) === String(c)
                      ? "btn"
                      : "btn btn-ghost"
                  }
                  disabled={!isHost}
        ...
        {room.game.matchDurationSeconds && (
          <p className="room-hint">
            제한 시간 {Math.round(room.game.matchDurationSeconds / 60)}분
          </p>
        )}
      </div>
      ...
      {sorted.map((p, i) => (
        <li key={p.id} className={p.id === me ? "me" : ""}>
          <span className="room-rank">
            {finished && p.rank ? `${p.rank}등` : `${i + 1}`}
          </span>
          <span className="room-name">
            <CharacterAvatar id={p.character} size={24} /> {p.nickname}
            {p.finished && !finished && (
              <span className="room-done"> 완료</span>
            )}
          </span>
  ...
  return (
    <div className="chat">
      <div className="chat-list" ref={listRef}>
        {messages.length === 0 && (
          <p className="room-hint">아직 대화가 없습니다.</p>
        )}
        {messages.map((m, i) =>
          m.system ? (
            ...
            <p
              key={i}
              className={m.playerId === me ? "chat-msg me" : "chat-msg"}
            >
